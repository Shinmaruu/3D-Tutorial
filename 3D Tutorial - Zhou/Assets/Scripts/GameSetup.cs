...
public class GameSetup : MonoBehaviour
{
    ...
    void Start()
    {
        ballRadius = ballPrefab.GetComponenet<SphereCollider>().radius = 100f;
    ...
    void PlaceCueBall()
    {
        GameObject ball = Instatiate(ballPrefab, cueBallPosition, Quaternion.identity);
        ball.GetComponent<Ball>().MakeCueBall();
    }

    void PlaceEightBall(Vector3 position)
    {
        GameObject ball = Instatiate(ballPrefab, position, Quaternion.identity);
